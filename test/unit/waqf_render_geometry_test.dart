...
TextSpan _single(String text, TextStyle base) =>
...
Future<ui.Image> _render(TextSpan span) async {
  ...
  canvas.scale(_ratio);
  canvas.drawRect(
    const Rect.fromLTWH(0, 0, 700, 200),
...
Future<Uint8List> _bytes(ui.Image image) async => (await image.toByteData(
...
void main() {
  testWidgets('D1 run split does not move stranded trailing marks', (
    tester,
  ) async {
    await tester.runAsync(() async {
      ...
      for (final entry in _samples.entries) {
        ...
        File(
          '$_outputDir/STRANDED__${entry.key}__single.png',
        ).writeAsBytesSync(await _bytes(singleImg));
        File(
          '$_outputDir/STRANDED__${entry.key}__d1.png',
        ).writeAsBytesSync(await _bytes(d1Img));

        report.writeln(
          '${entry.key}: differing px=$differing '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'D1 split changed glyph positions; the colour split must not '
  ...
  testWidgets('the shipping pipeline renders the approved D1 arrangement', (
    ...
    await tester.runAsync(() async {
      ...
      for (final entry in samples.entries) {
        ...
        File(
          '$_outputDir/SHIPPING__${entry.key}.png',
        ...
        expect(
          ...
          0,
          reason:
              '${entry.key}: the shipping pipeline no longer matches the '
