...
void main() {
  test(
    'scan shortest examples for remaining manual rules',
    () async {
      ...
      final best =
          <TajweedRule, ({int surah, int ayah, int score, String text})>{};
          ...
          for (final ayah in ayahs) {
            final rules = <TajweedRule>{
              for (final word in ayah.words)
                ...word.spans.map((span) => span.rule),
            };
            for (final rule in targets) {
              if (!rules.contains(rule)) continue;
              final normalized = ayah.arabic
      ...
      for (final rule in targets) {
        ...
        // ignore: avoid_print
        print(
          '${rule.name}: ${hit == null ? 'no match found' : '${hit.surah.toString().padLeft(3, '0')}${hit.ayah.toString().padLeft(3, '0')} score=${hit.score} text=${hit.text}'}',
        );
      }
    ...
    skip:
        'Utility scan only; izhar, shaddah, and waqf require manual curation because upstream tajweed spans do not expose them reliably.',
  );
}
